define([
    ...
    function Engine() {
        ...
        self.userBalanceBtc = 0;
        self.userBalanceClam = 0;
        self.userInvestedBtc = 0;
        self.userInvestmentProfitBtc = 0;
        self.userProfitBtc = 0;
        self.userInvestedClam = 0;
        self.userInvestmentProfitClam = 0;
        self.userProfitClam = 0;
        self.userProfitAmountClam = 0;
        self.userWageredBtc = 0;
        self.userWageredClam = 0;

        self.siteProfitAmountBtc = 0;
        self.siteInvestedBtc = 0;
        self.siteProfitPercentageBtc = 0;
        self.siteProfitAmountBtc = 0;
        self.siteWageredBtc = 0;
        self.siteProfitAmountClam = 0;
        self.siteInvestedClam = 0;
        self.siteProfitPercentageClam = 0;
        self.siteProfitAmountClam = 0;
        self.siteWageredClam = 0;
        ...
        */
        ...
        self.ws.on('game_started', function(bets) {
            ...
            Object.keys(bets).forEach(function(username) {
                if (self.username === username)
                    self.userBalanceClam -= bets[username];
        ...
        self.ws.on('game_starting', function(info) {
            ...
            self.siteBankroll = info.site_bankroll;

            self.ws.emit('get_stats');
        ...
        self.ws.on('cashed_out', function(resp) {
            ...
            //self.totalCashed += Math.round((self.playerInfo[resp.username].bet * resp.stopped_at / 100) - (self.playerInfo[resp.username].bet * .01));

            if (self.username === resp.username) {
                self.cashingOut = false;
                self.userBalanceClam += self.playerInfo[resp.username].bet * resp.stopped_at / 100;
                self.userState = 'WATCHING';
            }
            ...
            requestOtt(function(err, ott) {
                ...
                self.ws.emit('join', { ott: ott },
                    function(err, resp) {
                        ...
                        self.userBalanceBtc = resp.balance_satoshis_btc;
                        self.userInvestedBtc = resp.balance_satoshis_invested_btc;
                        self.userBalanceClam = resp.balance_satoshis_clam;
                        self.userInvestedClam = resp.balance_satoshis_invested_clam;

                        self.siteBankroll = resp.site_bankroll;
                        self.chat = resp.chat;
        ...
        self.ws.on('stats_update', function(data){
            if(data) {

                self.userBalanceBtc = data.userBalanceBtc;
                self.userInvestedBtc = data.userInvestedBtc;
                self.userInvestmentProfitBtc = data.userProfitAmountBtc;
                self.userProfitAmountBtc = data.netProfitBtc;
                self.userWageredBtc = data.userWageredBtc;
                self.siteInvestedBtc = data.siteInvestedBtc;
                self.siteProfitPercentageBtc = data.siteProfitPercentageBtc;
                self.siteProfitAmountBtc = data.siteProfitAmountBtc;
                self.siteWageredBtc = data.siteWageredBtc;

                self.userBalanceClam = data.userBalanceClam;
                self.userInvestedClam = data.userInvestedClam;
                self.userInvestmentProfitClam = data.userProfitAmountClam;
                self.userProfitAmountClam = data.netProfitClam;
                self.userWageredClam = data.userWageredClam;
                self.siteInvestedClam = data.siteInvestedClam;
                self.siteProfitPercentageClam = data.siteProfitPercentageClam;
                self.siteProfitAmountClam = data.siteProfitAmountClam;
                self.siteWageredClam = data.siteWageredClam;
           }

           self.trigger('stats_update');
        });
    ...
    Engine.prototype.doBet =  function(amount, autoCashOut, callback) {
        ...
        this.ws.emit('place_bet', amount, autoCashOut, function(error) {
            ...
            if (error) {
                ...
                if (error !== 'GAME_IN_PROGRESS' && error !== 'ALREADY_PLACED_BET' || error !== 'NOT_ENOUGH_BANKROLL') {
